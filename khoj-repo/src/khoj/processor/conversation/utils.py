...
def generate_chatml_messages_with_context(
    ...
    for chat in chat_history:
        ...
        if not is_none_or_empty(chat.context):
            references = "\n\n".join(
                {
                    f"# URI: {getattr(item, 'uri', None) or getattr(item, 'file', None)}\n## Context: {getattr(item, 'context_summary', None)}\n\n## {getattr(item, 'compiled', None)}\n"
                    if getattr(item, 'context_summary', None)
                    else f"# URI: {getattr(item, 'uri', None) or getattr(item, 'file', None)}\n## {getattr(item, 'compiled', None)}\n"
