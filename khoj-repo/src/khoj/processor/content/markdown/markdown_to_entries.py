...
class MarkdownToEntries(TextToEntries):
    ...
    def process(self, files: dict[str, str], user: KhojUser, regenerate: bool = False) -> Tuple[int, int]:
        ...
        # Generate context summaries for each chunk if contextual chunking is enabled
        if getattr(settings, 'CONTEXTUAL_CHUNKING_ENABLED', False) and current_entries:
            with timer("Generate context summaries for markdown entries", logger):
                try:
                    if state.openai_client:
                        chunker = ContextualChunker(llm_client=state.openai_client)
                        for i, entry in enumerate(current_entries):
                            try:
                                summary = chunker.summarize_chunk(entry.raw)
                                if summary:
                                    entry.context_summary = summary
                                if (i + 1) % 10 == 0:
                                    logger.info(f"Generated context summaries for {i + 1}/{len(current_entries)} entries")
                            except Exception as e:
                                logger.warning(f"Failed to generate context summary for entry {i}: {e}")
                                # Leave context_summary as None on error
                                continue
                        logger.info(f"Completed generating context summaries for {len(current_entries)} markdown entries")
                    else:
                        logger.warning("Contextual chunking enabled but openai_client not available")
                except Exception as e:
                    logger.error(f"Error during contextual chunking: {e}")
