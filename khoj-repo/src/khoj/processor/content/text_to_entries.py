...
class TextToEntries(ABC):
    ...
    def split_entries_by_max_tokens(
        entries: List[Entry],
        max_tokens: int = None,
        max_word_length: int = 500,
        raw_is_compiled: bool = False,
        chunk_sizes: List[int] = None,
    ) -> List[Entry]:
        """Split entries if compiled entry length exceeds the max tokens supported by the ML model.

        Supports multi-scale chunking by providing multiple chunk sizes. Each entry is chunked
        at each scale, and chunks are tagged with their chunk_scale for later retrieval.
        ...
        Returns:
            List of Entry objects with chunks from all scales, each tagged with chunk_scale
        """
        ...
        # Determine chunk_scale label: 'default' for single size, size as string for multiple
        use_default_scale = len(chunk_sizes) == 1
        ...
        for entry in entries:
            ...
            # Generate a unique corpus_id for all chunks from this original entry
            corpus_id = uuid.uuid4()

            # Process each chunk size scale
            for chunk_size in chunk_sizes:
                # Split entry into chunks of chunk_size
                ...
                # Determine chunk_scale for this scale
                chunk_scale = "default" if use_default_scale else str(chunk_size)
                ...
                for chunk_index, compiled_entry_chunk in enumerate(chunked_entry_chunks):
                    ...
                    if chunk_index > 0 and entry.heading:
                        # Snip heading to avoid crossing chunk_size limit
                    ...
                    cleaned_raw = compiled_entry_chunk if raw_is_compiled else TextToEntries.clean_field(entry.raw)
                    cleaned_heading = TextToEntries.clean_field(entry.heading)
                    cleaned_file = TextToEntries.clean_field(entry.file)
