...
def rrf_fuse(results_lists: List[List[dict]], k: int = 60, limit: int = 10) -> List[dict]:
    """
    Reciprocal Rank Fusion (RRF) for combining multiple ranked result lists.

    RRF formula: score = sum(1 / (k + rank + 1)) for each result list
    Results are deduplicated by ID and sorted by final RRF score descending.
    ...
    Returns:
        List of fused results sorted by RRF score descending, limited to `limit` items
    """
    if not results_lists:
        return []

    # Track RRF scores and result data by unique ID
    ...
    # Sort by RRF score descending and return top `limit` results
    sorted_ids = sorted(scores.keys(), key=lambda x: scores[x], reverse=True)
    top_ids = sorted_ids[:limit]

    # Return results in sorted order with their RRF scores included
    fused_results = []
...
def rrf_fuse_multi(
    ...
    """
    Multi-source Reciprocal Rank Fusion (RRF) for combining ranked results from multiple sources.

    This function extends rrf_fuse() to support named sources with optional weighting,
    enabling multi-scale retrieval fusion (e.g., combining dense_512, dense_1024, sparse_512, etc.)

    RRF formula: score = sum(weight * 1 / (k + rank + 1)) for each source
    Results are deduplicated by ID and sorted by final RRF score descending.

    Args:
        sources: Dict where keys are source names (e.g., 'dense_512', 'sparse_1024') and
                 values are lists of result dicts, each with 'id' or 'entry' key
        k: RRF constant (default 60), higher values give more weight to lower-ranked items
        limit: Maximum number of results to return (default 10)
        source_weights: Optional weights for each source (e.g., {'dense_512': 1.0, 'sparse_1024': 0.8})
                        Sources without explicit weights default to 1.0

    Returns:
        List of fused results sorted by RRF score descending, limited to `limit` items.
        Each result includes:
        - All original result fields
        - 'rrf_score': float - final fused RRF score
        - 'source_contributions': Dict[str, float] - contribution from each source
    """
    if not sources:
        return []

    # Track RRF scores, contributions, and result data by unique ID
    scores: dict[str, float] = {}
    ...
    for source_name, results in sources.items():
        ...
        weight = source_weights.get(source_name, 1.0) if source_weights else 1.0
        ...
        for rank, result in enumerate(results):
            ...
            # Accumulate score across all sources
            scores[result_id] = scores.get(result_id, 0.0) + rrf_contribution

            # Track individual source contributions
            if result_id not in contributions:
                contributions[result_id] = {}
            contributions[result_id][source_name] = rrf_contribution
    ...
    # Return results in sorted order with RRF scores and source contributions
    fused_results = []
    for result_id in top_ids:
        result = results_by_id[result_id].copy()
        result["rrf_score"] = scores[result_id]
        result["source_contributions"] = contributions.get(result_id, {})
        fused_results.append(result)
