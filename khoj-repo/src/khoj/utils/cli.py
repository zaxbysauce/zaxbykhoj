...
def _setup_parser(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Run Khoj in single user mode with no login required. Default: True. Useful for personal use or testing.",
    )
    parser.add_argument(
        "--no-anonymous-mode",
        action="store_false",
        dest="anonymous_mode",
        help="Disable anonymous mode and require authentication. Set GOOGLE_CLIENT_ID or RESEND_API_KEY to enable auth.",
    )
    parser.add_argument(
        "--non-interactive",
        action="store_true",
