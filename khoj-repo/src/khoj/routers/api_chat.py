...
async def event_generator(
    ...
    if ConversationCommand.Notes in conversation_commands:
        try:
            ...
            transformer = QueryTransformer(state.openai_client)
            if settings.QUERY_TRANSFORM_ENABLED:
                query_variants = transformer.transform(q)
            else:
                query_variants = [q]

            # Search documents for each query variant
            variant_results = []
            for variant in query_variants:
                ...
                async for result in search_documents(
                    ...
                    if isinstance(result, dict) and ChatEvent.STATUS in result:
                        ...
                        variant_references.extend(result[0])
                        inferred_queries.extend(result[1])
                        defiltered_query = result[2]

                # CRAG evaluation gating
                if variant_references:
                    max_similarity = max([r.get("score", 0) for r in variant_references], default=0)
                    if settings.CRAG_ENABLED and max_similarity < 0.75:
                        evaluator = RetrievalEvaluator(state.openai_client)
                        evaluation = evaluator.evaluate(variant, variant_references)
                        if evaluation == RetrievalEvaluation.NO_MATCH:
                            variant_references.clear()
                            logger.warning(f"CRAG evaluation cleared references for query: {variant}")

                variant_results.append(variant_references)

            # RRF fusion of results from all query variants
            if len(query_variants) > 1:
                compiled_references = rrf_fuse(variant_results, k=60, limit=(n or 7))
            else:
                compiled_references = variant_results[0] if variant_results else []
        except Exception as e:
