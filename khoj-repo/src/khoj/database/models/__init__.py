...
class LdapConfig(DbBaseModel):
    ...
    server_url = models.CharField(
        max_length=200,
        help_text="LDAP server URL (e.g., ldaps://ldap.example.com:636)"
    )
    ...
    user_search_filter = models.CharField(
        ...
        help_text="LDAP filter template for finding users"
    )
    use_tls = models.BooleanField(
        default=True,
        help_text="Use TLS/SSL for LDAP connections"
    )
    tls_verify = models.BooleanField(
        default=True,
        help_text="Verify TLS certificates"
    )
    tls_ca_bundle_path = models.CharField(
        max_length=500,
        null=True,
        blank=True,
        default=None,
        help_text="Path to custom CA bundle for TLS verification (optional)"
    )
...
class SearchModelConfig(DbBaseModel):
    ...
    # Alpha weight for hybrid search (sparse-dense), 0 = keyword only, 1 = dense only
    hybrid_alpha = models.FloatField(default=0.6)
    # Whether to enable hybrid search combining keyword and dense retrieval
    hybrid_enabled = models.BooleanField(default=True)
...
class Entry(DbBaseModel):
    ...
    search_vector = SearchVectorField(
        null=True,
        blank=True,
        help_text="PostgreSQL full-text search vector for efficient text search",
    )
    chunk_scale = models.CharField(
        max_length=16,
        default="default",
        blank=True,
        null=True,
    ...
    context_summary = models.TextField(
        ...
        blank=True,
        default=None,
        help_text="AI-generated context summary for improved retrieval (contextual chunking)",
    )
