...
python -m pip install 'khoj[local]'
 ...
 py -m pip install 'khoj[local]'
      ...
      The following CLI options have limitations on Windows:

      | CLI Option | Windows Support | Notes |
      |------------|-----------------|-------|
      | `--socket` | ❌ Not Available | UNIX sockets are not supported on Windows. Use `--host` and `--port` instead. |
      | `--host` | ✅ Supported | Use `127.0.0.1` or `0.0.0.0` |
      | `--port` | ✅ Supported | Default: `42110` |
      | `--sslcert` | ✅ Supported | Path to SSL certificate file |
      | `--sslkey` | ✅ Supported | Path to SSL key file |
      | `--anonymous-mode` | ✅ Supported | Run without authentication |
      | `--non-interactive` | ✅ Supported | Non-interactive mode |

      :::tip[Windows Path Format]
      When specifying file paths on Windows, use either:
      - Backslashes: `C:\path\to\file`
      - Forward slashes: `C:/path/to/file` (recommended for compatibility)
      :::

      :::tip[PowerShell Environment Variables]
      Set environment variables in PowerShell using:
      ```powershell
      $env:VAR_NAME="value"
      ```
      Or for persistent variables, add to User or System Environment Variables in System Properties.
      :::
    </TabItem>
    <TabItem value="linux" label="Linux">
      <Tabs groupId="gpu" queryString="gpu">
