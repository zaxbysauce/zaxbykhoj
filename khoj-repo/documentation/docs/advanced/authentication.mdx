...
By default, Khoj runs in anonymous mode (no authentication required) for easy personal use. This is suitable for single-user setups or testing. If you want to enable authentication for multi-user access, see the options below.

:::tip[Note]
Use the `--no-anonymous-mode` flag to require authentication. Set up either [Magic Links](#using-magic-links) or [Google OAuth](#using-google-oauth) before disabling anonymous mode.
:::
...
That's it! That should be all you have to do. Now, when you reload Khoj with `--no-anonymous-mode`, you should be able to use your Google account to sign in.

## Anonymous Mode Options

Khoj provides flexibility for how authentication is handled:

- **Default**: Runs with anonymous mode enabled (single-user, no login required)
- `--anonymous-mode`: Explicitly enable anonymous mode (same as default)
- `--no-anonymous-mode`: Disable anonymous mode and require authentication

### Migration for Existing Deployments

If you were previously running Khoj without explicit authentication settings and want to keep the old behavior (authentication required), add `--no-anonymous-mode` to your startup command and configure one of the authentication methods above.
